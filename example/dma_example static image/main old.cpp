...
// Example DMA complete callback
void dmaCompleteCallback() {
    // This runs in interrupt context! On Teensy, Serial.print is safe but keep it short.
...
void setup() {
  ...
  // Register DMA complete callback
  display.dmaDriver.setDmaCompleteCallback(dmaCompleteCallback);
...
void loop() {
    ...
    // Do NOT call draw_framebuffer_interruptible unconditionally here anymore.
    // The DMA driver will handle the state machine internally.
